...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        # https://www.notion.so/anish13/Stanford-224N-f79c860379484008a2e3e68fe0277526
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        # still dont fully understasnd this
        
        #Encoding
        src_emb = self.model_embeddings.source(source_padded) # X, (src_len, b, e), source_padded is sorted
        src_emb = nn.utils.rnn.pack_padded_sequence(src_emb, source_lengths) # (src_len, b, e)
        enc_hiddens, (last_hidden, last_cell) = self.encoder(src_emb) # (src_len, b, h*2), ((2, b, h), (2, b, h))
        enc_hiddens, _ = nn.utils.rnn.pad_packed_sequence(enc_hiddens, batch_first=True) # (b, src_len, h*2)
        # Decoding:
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        # dont fully undertand this either. Here we create the attention layer
        enc_hiddens_proj = self.att_projection(enc_hiddens) # (b, src_len, h)
        tgt_embs = self.model_embeddings.target(target_padded) # (tgt_len, b, e), Y
