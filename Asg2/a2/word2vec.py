...
def naiveSoftmaxLossAndGradient(
    ...
    gradOutsideVecs[outsideWordIdx] = (outsideSoftmaxVec[outsideWordIdx] - 1) * centerWordVec # w != o
...
def negSamplingLossAndGradient(
    ...
    scores = outsideVectors[indices] @ centerWordVec # shape: (K+1,)
    sigmoid_neg_score_ks = np.reshape(sigmoid(-scores[1:]), (-1, 1)) # shape: (K, 1)
    ...
    gradCenterVec = (sigmoid(scores[0]) - 1) * outsideVectors[outsideWordIdx] + np.sum(tmp_k, axis=0) # (d,)
